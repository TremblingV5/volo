...
use crate::{
    ...
    error::{
        ClientError,
        client::{
            Result, body_not_replayable, invalid_redirect_location, missing_location,
            too_many_redirects,
        },
    },
    request::Request,
    ...
    async fn call(&self, cx: &mut ClientContext, req: Request<B>) -> Result<Self::Response> {
        ...
        loop {
            ...
            let location = match resp.headers().get(LOCATION) {
                ...
                None => return Err(missing_location()),
            };

            let (target, uri) = resolve_redirect(cx.target(), &parts.uri, &location)
...
mod tests {
    ...
    #[tokio::test]
    async fn missing_location_errors() {
        let mock = RedirectMock::default();
        let client = mock_client(&mock, 10);
        ...
        assert!(
            err.to_string().contains("missing Location header"),
            "got: {err}"
        );
        assert_eq!(mock.hops().len(), 1);
    }
